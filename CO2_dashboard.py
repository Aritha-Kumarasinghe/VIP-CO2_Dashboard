...
@app.callback(
    [dash.dependencies.Output('co2-time-series', 'figure'),
     ...
     dash.dependencies.Output('top_CO2_value', 'children')],
...
def update_graph(time):
    ...
    return fig, f"The most recent CO2 readings at {df_room['time'].iloc[-1]} on {df_room['date'].iloc[-1]}",f"Room-456 : {df_room['CO2'].iloc[-1]}"
